...
if not combined_dataset.empty:
    ...
    cols_to_drop = ['Peça', 'Part', 'Scan', 'Iteração']
    rename_dict = {
        ...
        'input angle': 'psi_CAD_deg',
        'input height': 'h_CAD_mm'
    }
    ...
    if (
        ...
        for psi_cad in unique_psi_cad:
            ...
            ax.scatter(
                [f"$\\Delta z={sd:.1f}$\n$d_t={td:.0f}$" for sd, td in zip(subset['sd_mm'], subset['d_t_mm'])],
                subset['psi_diff'],
                label=f"$\\Psi_{{CAD}}={psi_cad:.0f} \\degree$"
            )
        ax.set_xlabel(r'($\Delta z$, $d_t$) combination', fontsize=18)
        ax.set_ylabel(r'$\Psi_{CAD} - \Psi_{m}$', fontsize=18)
        #ax.set_title(r"Difference between $\Psi_{CAD}$ and average $\Psi_{m}$ for each $\Delta z$, $d_t$", fontsize=20)

        # Make room at the bottom and place the legend as a single horizontal line below the x-axis label
        ncols = max(1, len(unique_psi_cad))
        fig.subplots_adjust(bottom=0.30)
        #ax.legend(loc='upper center', bbox_to_anchor=(0.5, -0.18), ncol=ncols, fontsize=13)
        ...
        plt.xlabel(r'($\Delta z$, $d_t$) combination', fontsize=18)
        plt.ylabel(r'$h_{CAD}- h_{m}$', fontsize=18)
        #plt.title(r"Difference between $h_{CAD}$ and average $h_{m}$ for each ($\Delta z$, $d_t$)", fontsize=20)
    ...
    for fold_idx in range(n_folds):
        ...
        # model, Y_pred, r2_scores, mses = linear_regression_and_3d_plots(
        ...
        # plot_3d_actual_vs_predicted(X_test, Y_test, Y_pred, output_targets, group_masks, latex_labels, model)
        ...
        # plt.ylabel(latex_labels['h_D_mm'], fontsize=14)
        # #plt.title('Scatter Plot for All Groups (Switched Axes)')
        # plt.grid(True, linestyle='--', alpha=0.7)
        # plt.legend(fontsize=13.8)
        ...
        #     ax.set_xlabel(latex_labels['psi_D_deg'], fontsize=14)
        #     ax.set_ylabel(latex_labels['h_D_mm'], fontsize=14)
        ...
        ######################################################################################
        # model_all, model_all_height = plot_linear_regression_all(
        ...
        #     # Plot each fitted tangent function and collect legend labels
        #     legend_labels = []
        #     for fit in fitted_params:
        #         popt = fit['params']
        #         sd = fit['sd_mm']
        #         td = fit['d_t_mm']
        #         #eq = f"$h = {popt[0]:.2f} \\cdot \\tan({popt[1]:.2f} \\cdot \\psi + {popt[2]:.2f}) + {popt[3]:.2f}$"
        #         label = rf"$\Delta z={sd:.2f},\ d_t={td:.0f}$"
        #         plt.plot(angle_line, tangent_func(angle_line, *popt), label=label)
        #         legend_labels.append(label)
        #     plt.xlabel(latex_labels['psi_D_deg'], fontsize=14)
        #     plt.ylabel(latex_labels['h_D_mm'], fontsize=14)
        #     #plt.title('Fitted Tangent Functions for All (sd_mm, d_t_mm) Combinations')
        #     plt.grid(True, linestyle='--', alpha=0.7)
        #     plt.legend(title="Fitted Tangent Equations", fontsize=14)
        #     plt.xticks(fontsize=14)
        #     plt.yticks(fontsize=14)
        #     plt.tight_layout()
        #     plt.show()

        # predict_and_plot_models(new_data, fitted_params, linear_params_h, linear_params_angle, prediction_table, fold_idx)
        ...
        results_csv_path = base_path+ r"\results\model_performance_final.csv"
        if os.path.exists(results_csv_path):
            os.remove(results_csv_path)
        results_df.to_csv(results_csv_path, index=False)
        print("Results saved to model_performance_final.csv")

        # After all folds, save the prediction table to a CSV file (override if exists)
        prediction_df = pd.DataFrame(prediction_table)
        predictions_csv_path = base_path+ r"\results\model_predictions_final.csv"
        ...
        print("Predictions saved to model_predictions_final.csv")
    ...
    all_params_csv_path = base_path+ r"\results\all_nn_hyperparameters_final.csv"
